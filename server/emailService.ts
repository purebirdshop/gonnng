...
export const emailService = {
  ...
  async sendWelcomeEmail(to: string, name: string, username: string): Promise<boolean> {
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; color: #111827; background-color: #ffffff; border-radius: 16px; border: 1px solid #e5e7eb;">
        ...
        <p style="font-size: 14px; line-height: 1.6; color: #4b5563; margin-bottom: 20px;">
          Your creator account (<strong>@${username}</strong>) is ready. You can now build projects, share updates, manage recipes, and connect with your creative circle.
        </p>
        <div style="text-align: center; margin: 32px 0;">
          <a href="${appUrl}" style="background-color: #FF5C00; color: #000000; font-weight: 700; font-size: 14px; padding: 12px 28px; text-decoration: none; border-radius: 10px; display: inline-block;">
            Open Your Workspace
          </a>
        </div>
        <hr style="border: 0; border-top: 1px solid #f3f4f6; margin: 24px 0;" />
        <p style="font-size: 12px; color: #9ca3af; text-align: center;">
          If you didn't create an account on Gonnng, please ignore this email.
        </p>
      </div>
    `;
    ...
    const result = await this.sendEmail({
      to,
      subject: 'Welcome to Gonnng! 🚀',
      html,
      text: `Welcome to Gonnng, ${name}! Your creator handle is @${username}. Open your workspace at ${appUrl}`
    });
